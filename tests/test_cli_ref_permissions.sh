...
printf '%s\n' "$output" | grep "\[PROVEN\] fn.store_preserves_write_permission.safety.2.ownership_present" >/dev/null
printf '%s\n' "$output" | grep "\[PROVEN\] fn.store_preserves_write_permission.safety.3.mutable_borrow_active" >/dev/null
printf '%s\n' "$output" | grep "\[PROVEN\] fn.store_preserves_write_permission.safety.4.memory_valid" >/dev/null
printf '%s\n' "$output" | grep "\[PROVEN\] fn.store_preserves_write_permission.safety.5.memory_write" >/dev/null
...
write_gate="$smt_dir/fn.store_preserves_write_permission.safety.5.memory_write.smt2"
