...
printf '%s\n' "$output" | grep "\[PROVEN\] fn.same_ref_loads_match.safety.1.memory_live" >/dev/null
printf '%s\n' "$output" | grep "\[PROVEN\] fn.same_ref_loads_match.safety.2.memory_valid" >/dev/null
printf '%s\n' "$output" | grep "\[PROVEN\] fn.same_ref_loads_match.safety.3.memory_live" >/dev/null
printf '%s\n' "$output" | grep "\[PROVEN\] fn.same_ref_loads_match.safety.4.memory_valid" >/dev/null
